...
// Originally taken from https://github.com/goji/httpauth.
//
// Unfortunately httpauth was not flexible enough for my needs (no
// multi-account support), hence this package exists.
...
func (b *basicAuth) authenticate(r *http.Request) bool {
	...
	if b.opts.AuthFunc != nil {
		result := b.opts.AuthFunc(string(creds[0]), string(creds[1]))
		return result
	}
